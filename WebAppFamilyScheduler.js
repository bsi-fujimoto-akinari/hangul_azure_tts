...
function h3FamilySchedulerHomeNext() {
  ...
  // Do not hold the Family Scheduler ScriptLock here.
  // prepareListeningBackendSet() owns its own lock around source
  // persistence and finalization and runs targeted audio between them.
...
function h3FamilySchedulerShadowTick() {
  ...
  // This suffix is only a date-local unique identity and is never
  ...
  throw new Error('FAMILY_SCHEDULER_LISTENING_SET_ID_EXHAUSTED');
}
...
function h3FsBuildListeningPrepare_(ss) {
  ...
  if(!Number.isInteger(next)||next<1){
    throw new Error('FAMILY_SCHEDULER_LISTENING_NEXT_SET_NO_INVALID');
  }
