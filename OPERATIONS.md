...
UXR-2B one-shot exception: for the explicitly authorized repair of `H3-20260915-01 / D4` Review audio only, the temporary repair step may run once from the automatic `workflow_run` path on the single audited main revision whose immediate parent is exactly `7bf95428859bedf071c3cd58b0a03421d6fe4b69`. The triggering Repository audit must be a successful push audit for that exact current-main SHA, `github.run_attempt` must equal `1`, the Apps Script HEAD source attestation must pass in the same job, and execution must still pass through `smoke_boundary ready=true`. This exception authorizes only `runUxr2bReviewAudioRepair`; it does not authorize any other automatic credentialed function execution and must be removed immediately after exact sidecar readback verifies the replacement.
