...
- Review priority uses `H3_REVIEW_PRIORITY_V4` and is globally normalized to 0–100. Each item starts from its stored result (`×=1.00`, `△=0.65`, `○=0.00`). Later learner evidence on the same `skill_id` updates that residual weakness only when it comes from a different set and different surface: a later `×` resets it to `1.00`, a later `△` to `0.65`, the first independent `○` after a non-correct result lowers it to `0.35`, and the second to `0.10`; later non-correct evidence resets this recovery. Two spaced corrects therefore stabilize/deprioritize but do not claim mastery. The mean residual weakness is soft-weighted by the existing 50:50 equal-surface/exam-share factor (`5L=40 / 5W=36 / 2R=12 / 2T=12`).
- Forgetting pressure is computed from each item's latest authoritative skill-evidence timestamp, not permanently from the originating set timestamp. For each item, `F_i = 1 - 2^(-d_i/14)`; HOME uses the mean `F_i` and `P = B + (100-B)×0.40×mean(F_i)`. `review_home_index_v1` V5 stores the derived `BASE_PRIORITY` plus source-traceable `PRIORITY_STATE_JSON`; committed learner logs remain the evidence authority.
- HOME history cards do not render the old textual `×n / ?n / 復習優先度 n` summary. They retain score/date plus a compact normalized priority bar used for `Priority` sorting. After all Review questions have been viewed and the learner taps `ホーム`, the Web App records only derived cooldown metadata (`LAST_REVIEWED_AT` + a stable per-Review-session `LAST_REVIEW_COMPLETION_KEY`) in the HOME index. Cooldown is presentation/order state only: 0–24h uses a cap rising linearly from 10 to 25, 24–96h recovers linearly from `min(P,25)` to `P`, and 96h+ uses `P` unchanged. HOME navigation never waits for this metadata write. Same-event replay is idempotent and does not advance the timestamp; a later reopened Review uses a new completion key. Only typed transient/transport failure may retry once; validation/integrity failure is never retried. Opening or partially viewing a Review does not trigger cooldown, and no score/history/scheduler state is changed.
- `ANSWERED_AT` is required for every ACTIVE Review row. Historical missing values have been backfilled; HOME no longer substitutes another set's timestamp. Missing or invalid `ANSWERED_AT` is an integrity failure and is not guessed.
- HOME history is served only from the derived `review_home_index_v1`. This index stores one ACTIVE row per provider/set plus precomputed dynamic priority state; it is not learner history or Review authority.
- `BASE_PRIORITY` and `PRIORITY_STATE_JSON` are refreshed after a committed answer/Answer Sync, not during HOME rendering. HOME recomputes only time-dependent forgetting and cooldown from that derived state.
