...
- HOME history cards do not render the old textual `×n / ?n / 復習優先度 n` summary. They retain score/date plus a compact normalized priority bar used for `Priority` sorting. After all Review questions have been viewed and the learner taps `ホーム`, the Web App records only `LAST_REVIEWED_AT`; HOME then applies a 45% immediate cooldown that recovers linearly over 72 hours. Opening or partially viewing a Review does not trigger cooldown, and no score/history/scheduler state is changed.
